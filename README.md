...
- Реализован паттерн "Модуль" на примере active-video-module.js, который отвечает на поведение активного видео и имеет только один публичный метод - toggleVideoActivity()
- Оптимизирован алгоритм определения освещенности. Теперь для анализа берутся только пиксели с заданным шагом, который не должен быть кратен ширине экрана, чтобы пиксели по высоте гарантированно образовывали диагонали. На картинке отмечены точки, по которым определяется освещенность.
![measure-brightness](https://user-images.githubusercontent.com/14029952/46919287-f3d3bb00-cfe5-11e8-9aa5-f4b6529ec3ee.JPG)
- Реализован алгоритм захвата движения (только в десктопной версии). С заданной частотой, анализируется кадр и попиксельно сравнивается с предыдущим. Определяется первый и последний пиксель, где разница интенсивности цвета превышает установленную дельту. Таким образом регулируя дельту интенсивности и частоту опроса можно регулировать поведение данного алгоритма.  
Область захвата движения отрисовывается на небольшом канвасе в левом верхнем углу видео. Такой формат было дешевле реализовать, а также удобно использовать как опорное изображение при минимальных значениях яркости/конрастности.
![movement](https://user-images.githubusercontent.com/14029952/46919252-5b3d3b00-cfe5-11e8-8338-d83aa5aa7244.JPG)
